<script lang="ts">
	...
	let patientData = {
		...
		affiliateNumber: '12458963',
		symptoms: ['Dolor de cabeza frecuente (2 días)', 'Fiebre leve (37.5° C)', 'Malestar general', 'Consulta seguimiento']
	};
	...
	let doctorVideoElement: HTMLVideoElement; // Video del médico (inset pequeño)
	let patientVideoElement: HTMLVideoElement; // Video del paciente (grande)

	// Streams de media
	let doctorStream: MediaStream | null = null; // Stream del médico (inset pequeño)
	let patientStream: MediaStream | null = null; // Stream del paciente (grande)
	let isLoadingCamera = false;

	// Función para iniciar la cámara del médico (inset pequeño)
	async function startDoctorCamera() {
		if (isLoadingCamera) return;
		
		try {
			isLoadingCamera = true;
			// Solicitar acceso a cámara y micrófono del médico
			...
			// Asignar el stream al elemento de video del médico
			if (doctorVideoElement) {
				doctorVideoElement.srcObject = doctorStream;
				// Forzar reproducción del video
				try {
					await doctorVideoElement.play();
					console.log('Video del médico iniciado correctamente');
				} catch (err) {
					console.error('Error al reproducir video:', err);
					// Intentar reproducir sin muted si falla
					doctorVideoElement.muted = false;
					await doctorVideoElement.play();
				}
			}
			videoEnabled = true;
	...
	// Función para detener la cámara del médico
	function stopDoctorCamera() {
		...
		if (doctorVideoElement) {
			doctorVideoElement.srcObject = null;
		}
	}
	...
	function updateAudioTrack() {
		if (doctorStream) {
			doctorStream.getAudioTracks().forEach((track: MediaStreamTrack) => {
	...
	onMount(async () => {
		...
		// Iniciar la cámara del médico automáticamente al cargar la página
		if (videoEnabled) {
			await startDoctorCamera();
		}
	});
	...
	onDestroy(() => {
		stopDoctorCamera();
		// Limpiar stream del paciente si existe en el futuro
		// Nota: patientStream se implementará cuando se agregue WebRTC para el paciente
		if (patientStream) {
		...
		if (patientVideoElement) {
			patientVideoElement.srcObject = null;
		}
	});
...
<svelte:head>
	<title>Telemedicina - SPS Salud</title>
	<meta name="description" content="Consulta de telemedicina - SPS Salud" />
</svelte:head>
...
<div class="min-h-screen bg-gray-100">
	...
	<div class="w-full flex justify-center bg-gray-100">
		...
		<div class="flex-1 bg-gray-900 relative flex flex-col">
			<!-- Vista previa del médico - pequeño en esquina superior derecha -->
			<div class="absolute top-2 right-2 sm:top-3 sm:right-3 md:top-4 md:right-4 z-10 w-24 h-20 sm:w-32 sm:h-24 md:w-48 md:h-36 lg:w-64 lg:h-48 bg-gray-800 rounded-lg overflow-hidden border-2 border-white shadow-lg">
				{#if doctorStream && videoEnabled}
					<!-- Vista previa del médico -->
					<video
				...
				{:else if isLoadingCamera}
					...
					<div class="w-full h-full flex items-center justify-center bg-gray-700">
						<div class="text-white text-center px-1">
							<div class="w-6 h-6 sm:w-8 sm:h-8 border-2 border-white border-t-transparent rounded-full animate-spin mx-auto mb-1"></div>
				...
				{:else}
					...
					<div class="w-full h-full flex items-center justify-center bg-gray-700">
						<div class="text-white text-center px-1">
							...
							<p class="text-[10px] sm:text-xs">{consultationData.doctor}</p>
						</div>
					</div>
				{/if}
			</div>

			<!-- Video del paciente - grande -->
			<div class="flex-1 flex items-center justify-center bg-gray-900 relative overflow-hidden">
				{#if patientStream}
					<!-- Video del paciente -->
					<video
						bind:this={patientVideoElement}
						autoplay
						playsinline
						class="w-full h-full object-cover"
					></video>
				{:else}
					...
					<div class="w-full h-full flex items-center justify-center bg-gray-800">
						<div class="text-white text-center px-4">
							...
							<p class="text-base sm:text-lg md:text-xl mb-1 sm:mb-2">{patientData.name}</p>
							<p class="text-xs sm:text-sm text-gray-400">Esperando conexión del paciente...</p>
						</div>
					</div>
				{/if}
			</div>
			...
			<div class="absolute bottom-3 sm:bottom-4 md:bottom-6 left-1/2 transform -translate-x-1/2 z-20">
				<div class="flex items-center gap-1.5 sm:gap-2 md:gap-3 bg-black/50 backdrop-blur-sm rounded-full px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-3">
					...
					<button
						...
						class="w-9 h-9 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full {videoEnabled && doctorStream ? 'bg-gray-700 hover:bg-gray-600' : 'bg-red-600 hover:bg-red-700'} {isLoadingCamera ? 'opacity-50 cursor-not-allowed' : ''} text-white flex items-center justify-center transition-colors"
						aria-label={videoEnabled && doctorStream ? 'Desactivar video' : 'Activar video'}
					>
						{#if isLoadingCamera}
							<div class="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
						{:else if videoEnabled && doctorStream}
